...
const Chart10 = () => {
  ...
  const options = {
    ...
    legendCallback: chart => {
      ...
      legendMarkup.push('<div className="kit__c9__chartLegend flex-shrink-0">')
      let legends = labels.map((label, index) => {
        const color = dataset.backgroundColor[index]
        return `<div className="d-flex align-items-center flex-nowrap mt-2 mb-2"><div className="tablet mr-3" style="background-color: ${color}"></div>${label}</div>`
      })
